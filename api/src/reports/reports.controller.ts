...
export class ReportsController {
    ...
    async summary(@Query() q: SummaryQueryDto): Promise<SummaryResponseDto> {
        const whereConv: any = { eventAt: { gte: q._fromDate ?? new Date(q.from), lte: q._toDate ?? new Date(q.to) } };
        const whereClick: any = { eventAt: { gte: q._fromDate ?? new Date(q.from), lte: q._toDate ?? new Date(q.to) } };

        if (q.network) {
            const n = await this.prisma.network.findFirst({ where: { name: q.network } });
            if (n) { whereConv.networkId = n.id; whereClick.networkId = n.id; }
        }
        if (q.sub) {
            const s = await this.prisma.subAffiliate.findFirst({ where: { code: q.sub } });
            if (s) { whereConv.subAffiliateId = s.id; whereClick.subAffiliateId = s.id; }
        }
        ...
        return { clicks, conversions, ctr, cvr, revenue: payout, payout, epc: clicks ? payout / clicks : 0 };
    }
    ...
    async bySub(
        ...
        const subs = await this.prisma.subAffiliate.findMany({ select: { id: true, code: true, name: true } });
        return subs.map(s => {
            const m = map[s.id] ?? { clicks: 0, conversions: 0, revenue: 0 };
            const ctr = m.clicks ? m.conversions / m.clicks : 0;
            return { subCode: s.code, subName: s.name, ...m, epc: m.clicks ? m.revenue / m.clicks : 0, ctr, cvr: ctr };
        });
    }
}
